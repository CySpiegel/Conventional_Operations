...
[_x,[
				"<img size=2 color='#7CC2FF' image='Screens\FOBA\talk_ca.paa'/><t font='PuristaBold' color='#7CC2FF'>Investigate",
				{
					...
					[(_this select 0), ( selectRandom ["Acts_CivilTalking_2", "Acts_CivilTalking_1", "Acts_Commenting_On_Fight_in", "Acts_CivilListening_2", "Acts_CivilListening_1", "Acts_CivilIdle_2", "Acts_CivilIdle_1"])] remoteExec ["playMove", (_this select 0)];					
				},
				nil,
				1.5,
				true,
...
[_x,[
				...
				{
			playSound3D [getMissionPath (selectRandom ["GoProne_1.ogg", "Halt.ogg", "Stop.ogg", "VehStop_2.ogg"]), player];
					(_this select 0) removeAction (_this select 2) ;			
...
{ {			_x removeAllEventHandlers "Killed";
			removeAllActions _x;
} foreach (allUnits select {side _x == independent && captive _x == false}); 
} remoteExec ["call", 0];			
...
} foreach (allUnits select {side _x == civilian && captive _x == false}); 
