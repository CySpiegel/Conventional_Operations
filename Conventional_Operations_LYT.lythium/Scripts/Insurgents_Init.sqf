...
if (_Chance == 1) then {
		_Road= selectRandom (_thisCenterTrigger nearRoads 2000)  ; 
		...
		_House = selectRandom (nearestObjects [_thisCenterTrigger, ["HOUSE"], 2000]select {count (_x buildingPos -1) > 3});  
...
_chance = selectRandom [1, 2] ;
if (count (allMapMarkers select {markerType _x == "o_inf" && (getMarkerPos _x distance _thisCenterTrigger < 2000)}) > 0) then {

_InsVillMrks = allMapMarkers select {markerType _x == "o_inf" && (getMarkerPos _x distance _thisCenterTrigger < 2000)}  ; 
_InsVillMrk = selectRandom InsVillMrks ;
				...
				_trgA = createTrigger ["EmptyDetector", (getPos getMarkerPos _InsVillMrk)];
				_trgA setTriggerArea [1000, 1000, 0, false, 200];
				_trgA setTriggerTimeout [1, 1, 1, true];
				_trgA setTriggerActivation ["WEST", "PRESENT", false];
				_trgA setTriggerStatements [
		...
		_Mount = selectRandom (nearestLocations [ _thisCenterTrigger, ["Mount"], 2000]);  
