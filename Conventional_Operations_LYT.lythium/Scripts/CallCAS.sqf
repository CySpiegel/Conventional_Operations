...
_AA = nearestobjects [_CAST,["O_SAM_System_01_F", "O_SAM_System_04_F"],3500];

if (count ( nearestobjects [player,["B_Radar_System_01_F", "I_E_Radar_System_01_F"],40000]) > 0 ) then {
	if ({alive _x} count _AA == 0 ) then {

_GSHPs = nearestobjects [player,[F_Heli_06_G, F_Heli_07_G],40000] select {alive _x == true  && (getMarkerPos _NearFOBMark) distance (position _x) < 500 };

	if ({alive _x} count _GSHPs > 0 ) then {
_GSHP = nearestobjects [(getMarkerPos _NearFOBMark),[F_Heli_06_G, F_Heli_07_G],40000] select 0;
_poss0 = getPos _GSHP ;
...
[_GSHPGroup,_CAST, 200] call BIS_fnc_taskPatrol;
...
_CAS_group_Veh = createVehicle [F_Heli_06_G, (getMarkerpos _CASStrt) , [], 0, "FLY"] ;
...
[_CAS_group,_CAST, 200] call BIS_fnc_taskPatrol;
_TRGTs = _CAST nearEntities [["Man","Car","Tank", "Air", "Ship", "LandVehicle"], 500] ; 
